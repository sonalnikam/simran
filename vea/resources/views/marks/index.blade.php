...
<ol class="breadcrumb">
            ...
            <li class="breadcrumb-menu">
                <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                    ...
                    <a class="btn btn-secondary" href="{{ url('/marks/'.$branch.'/'.$batch->id.'/'.$standard.'/newmarks') }}"><i class="icon-plus"></i> &nbsp;Create Marks</a>
                </div>
            </li>
</ol>
@endsection
    ...
    <div class="card-block">
        <div class="row">
            ...
            <div class="col-xs-5">
                ...
                @endphp
                <form action="{{ url('/marks/'.$branch.'/'.$batch->id.'/'.$standard.'/search') }}" method="get" id="frmserch">
                    {{ csrf_field() }}
                                            ...
                                            <tbody id="tasks-list">
                                             @foreach ($marks as $mark)
                                                <tr>
                                                    ...
                                                    <td>
                                                   <div> 
                                                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="window.location.href='{{ url('/marks/'.$branch.'/'.$mark->id.'/'.$batch->id.'/'.$standard.'/editmarks') }}'">Edit</button>
                                                    <button type="button" class="btn btn-outline-danger btn-sm" onclick="javascript:confirmDelete('{{ url('/marks/'.$branch.'/'.$mark->id.'/'.$batch->id.'/'.$standard.'/deletemarks') }}')">Delete</button>
                                                    </div>
                                                    </td>
                                                    <td>
                                                   <div class="float-xs-right"> 
                                                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="window.location.href='{{ url('/marks/'.$branch.'/'.$mark->id.'/'.$batch->id.'/'.$standard.'/addstudentmarks') }}'">Add</button>
                                                    <button type="button" class="btn btn-outline-success btn-sm" onclick="window.location.href='{{ url('/marks/'.$branch.'/'.$mark->id.'/'.$batch->id.'/'.$standard.'/liststudentmarks') }}'">List</button>
                                                    </div>
                                                    </td>
                                                </tr>
                                            @endforeach
